...
func getTealegGenerator() TealegGenerator {
	return TealegGenerator{
		openedFile:   nil,
		filename:     "",
		currentSheet: "",
		currentCol:   0,
...
func generateXlsxFile(html string, outputFilename string, batchSize int) string {
	...
	for i, table := range tables {
		...
		for rowsProceeded < len(rows) {
			...
			processTableRows(rows, generator, len(theadTrs), rowsProceeded, packSize)
...
func processTableRows(rows []xml.Node, generator *ExcelizeGenerator, headerOffset int,  offset int, rowsNumber int) {
	if offset >= len(rows) {
		return
	}
	...
	for i := offset; i <= (offset + rowsNumber - 1); i++ {
		...
		for _, theadTh := range theadTrs {
			...
			generator.SetCellValue(theadTh.Content())
			colIndex++
		}
		...
		for _, td := range cells {
			...
			generator.SetCellValue(td.Content())
			//xlsxCell = nil
		}
